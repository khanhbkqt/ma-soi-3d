...
export function parseActionResponse(
    ...
    if (jsonMatch) {
      ...
      if (parsed.target && parsed.target !== 'skip') {
        ...
        if (match) parsed.target = match;
          ...
          parsed.target = validNames[Math.floor(Math.random() * validNames.length)];
      }
      ...
      for (const key of ['target1', 'target2', 'player1', 'player2']) {
        if (parsed[key]) {
          ...
          if (match) parsed[key] = match;
            ...
            parsed[key] = validNames[Math.floor(Math.random() * validNames.length)];
        }
      }
      return parsed;
    }
  ...
  return {
    target: validNames.length ? validNames[Math.floor(Math.random() * validNames.length)] : null,
  };
}
