...
function CelestialBodies({ sunPos, moonPos, sunCol, sunScale, moonScale }: { sunPos: THREE.Vector3; moonPos: THREE.Vector3; sunCol: THREE.Color; sunScale: number; moonScale: number }) {
  ...
  return (
    <>
      <group ref={sunRef}>
        <mesh><sphereGeometry args={[1, 16, 16]} /><meshBasicMaterial color={curSunCol.current} /></mesh>
        <mesh><sphereGeometry args={[1, 16, 16]} /><meshBasicMaterial color={curSunCol.current} transparent opacity={0.15} /></mesh>
      </group>
      <group ref={moonRef}>
        <mesh><sphereGeometry args={[0.8, 16, 16]} /><meshBasicMaterial color="#ddeeff" /></mesh>
        <mesh><sphereGeometry args={[1.2, 16, 16]} /><meshBasicMaterial color="#aaccff" transparent opacity={0.1} /></mesh>
      </group>
    </>
  );
}
...
function Clouds() {
  ...
  const clouds = useMemo(() => Array.from({ length: 6 }, (_, i) => ({
    x: (Math.random() - 0.5) * 30, y: 12 + Math.random() * 5, z: -8 + (Math.random() - 0.5) * 15,
    scale: 1.5 + Math.random() * 2, speed: 0.1 + Math.random() * 0.15, offset: Math.random() * 100,
  })), []);
    ...
    <group ref={ref}>
      {clouds.map((c, i) => (
        <group key={i} position={[c.x, c.y, c.z]} scale={c.scale}>
          {[[-0.5, 0, 0], [0.3, 0.15, 0.2], [0, 0.1, -0.3]].map((p, j) => (
...
export default function VillageScene() {
  ...
  const cur = useRef({
    ambient: target.ambient, ambientCol: new THREE.Color(target.ambientCol),
    dir: target.dir, dirCol: new THREE.Color(target.dirCol),
    bg: new THREE.Color(target.bg),
    campfire: target.campfire, campfireCol: new THREE.Color(target.campfireCol),
  ...
  useFrame((state, dt) => {
    ...
    if (rimRef.current) { rimRef.current.intensity = c.dir * 0.3; rimRef.current.color.copy(c.dirCol); }
  ...
  return (
    <>
      ...
      <directionalLight ref={rimRef} position={[-8, 10, -10]} intensity={cur.current.dir * 0.3} color={cur.current.dirCol} />
