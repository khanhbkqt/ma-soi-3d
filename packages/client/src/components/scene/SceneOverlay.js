...
const PHASE_OVERLAYS = {
  [`${Phase.Lobby}_${Phase.Night}`]: {
    emoji: '🌙',
    text: 'Đêm Đầu Tiên',
    sub: 'Ngôi làng chìm vào bóng tối...',
    gradient: ['#1a1a3a', '#334466'],
  },
  [`${Phase.Night}_${Phase.Dawn}`]: {
    emoji: '🌅',
    text: 'Bình Minh',
    sub: 'Mặt trời lên...',
    gradient: ['#ff8c00', '#ffcc44'],
  },
  [`${Phase.Dawn}_${Phase.Day}`]: {
    emoji: '☀️',
    text: 'Ban Ngày',
    sub: 'Thảo luận bắt đầu',
    gradient: ['#4488cc', '#88ccff'],
  },
  [`${Phase.Day}_${Phase.Dusk}`]: {
    emoji: '🌇',
    text: 'Hoàng Hôn',
    sub: 'Bỏ phiếu đề cử...',
    gradient: ['#cc4400', '#ff6622'],
  },
  [`${Phase.Dusk}_${Phase.Judgement}`]: {
    emoji: '⚖️',
    text: 'Phán Xét',
    sub: 'Ai sẽ bị treo cổ?',
    gradient: ['#aa2222', '#cc4444'],
  },
    ...
    for (const e of recent) {
      ...
      if (e.type === GameEventType.GameStarted) {
        ...
        entry = {
          ...
          emoji: '⚖️',
          text: 'Hành Hình',
          sub: `${e.data.accusedName} đã bị treo cổ`,
          gradient: ['#5a0a0a', '#aa2222'],
        };
      } else if (e.type === GameEventType.PlayerDied) {
        entry = {
          id: key,
          emoji: '💀',
          text: `${e.data.playerName} Đã Chết`,
          sub:
            e.data.cause === 'wolf_kill'
              ? 'Bị sói cắn'
              : e.data.cause === 'witch_kill'
                ? 'Bị đầu độc'
                : 'Đã ra đi',
