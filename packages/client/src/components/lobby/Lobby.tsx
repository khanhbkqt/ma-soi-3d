...
const ROLE_COLORS: Record<Role, string> = {
  [Role.Werewolf]: 'text-red-400', [Role.AlphaWolf]: 'text-red-500', [Role.WolfCub]: 'text-red-300',
  [Role.Villager]: 'text-green-400', [Role.Seer]: 'text-purple-400', [Role.ApprenticeSeer]: 'text-purple-300',
  [Role.Witch]: 'text-emerald-400', [Role.Hunter]: 'text-orange-400', [Role.Guard]: 'text-blue-400',
  [Role.Cupid]: 'text-pink-400', [Role.Fool]: 'text-yellow-400',
};
...
const ROLE_EMOJI: Record<Role, string> = {
  [Role.Werewolf]: '🐺', [Role.AlphaWolf]: '🐺👑', [Role.WolfCub]: '🐺🍼',
  [Role.Villager]: '👤', [Role.Seer]: '🔮', [Role.ApprenticeSeer]: '🔮📚',
  [Role.Witch]: '🧪', [Role.Hunter]: '🏹', [Role.Guard]: '🛡️',
...
const PROVIDER_DEFAULTS: Record<string, Partial<ProviderConfig>> = {
  ...
  'openai': { name: 'OpenAI', model: 'gpt-4o-mini', baseUrl: 'https://api.openai.com/v1', apiKey: '' },
  'anthropic': { name: 'Anthropic', model: 'claude-sonnet-4-20250514', baseUrl: '', apiKey: '' },
  'ollama': { name: 'Ollama Local', model: 'llama3.2', baseUrl: 'http://localhost:11434', apiKey: '' },
};
...
const MODEL_PLACEHOLDERS: Record<string, string> = {
  'openai-compatible': 'model-name', 'openai': 'gpt-4o-mini', 'anthropic': 'claude-sonnet-4-20250514', 'ollama': 'llama3.2',
};

const URL_PLACEHOLDERS: Record<string, string> = {
  'openai-compatible': 'http://localhost:1234/v1', 'openai': 'https://api.openai.com/v1', 'anthropic': '', 'ollama': 'http://localhost:11434',
};

function ProviderForm({ initial, onSave, onCancel, saveLabel }: {
  initial: Partial<ProviderConfig>; onSave: (p: Partial<ProviderConfig>) => void; onCancel: () => void; saveLabel: string;
}) {
  ...
  return (
    <div className="bg-gray-900 rounded-lg p-4 mb-4 border border-gray-600 space-y-3">
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          <label className="block text-xs text-gray-400 mb-1">Base URL {type === 'openai-compatible' ? '(bắt buộc)' : ''}</label>
          <input value={form.baseUrl || ''} onChange={e => setForm({ ...form, baseUrl: e.target.value })}
            placeholder={URL_PLACEHOLDERS[type]}
            className="w-full bg-gray-800 border border-gray-600 rounded px-3 py-2 text-white text-sm" />
          {type === 'openai-compatible' && <p className="text-[10px] text-gray-500 mt-1">LM Studio, vLLM, Groq, Together, Fireworks, v.v.</p>}
        </div>
        <div>
          <label className="block text-xs text-gray-400 mb-1">Model</label>
          <input value={form.model || ''} onChange={e => setForm({ ...form, model: e.target.value })}
            list={`models-${type}`} placeholder={MODEL_PLACEHOLDERS[type]}
            className="w-full bg-gray-800 border border-gray-600 rounded px-3 py-2 text-white text-sm" />
        ...
        {type !== 'ollama' && (
          <div className="col-span-2">
            <label className="block text-xs text-gray-400 mb-1">API Key {type === 'openai-compatible' ? '(nếu cần)' : ''}</label>
      ...
      <div className="flex gap-2">
        <button onClick={() => onSave(form)} className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded text-sm transition">{saveLabel}</button>
        <button onClick={onCancel} className="bg-gray-700 hover:bg-gray-600 text-white px-4 py-2 rounded text-sm transition">Hủy</button>
      </div>
    </div>
  );
}
...
export default function Lobby() {
  ...
  useEffect(() => {
    setEnabledRoles(prev => {
      ...
      const newRoles = defaults.filter(r => !TOGGLEABLE_ROLES.some(t => t.role === r && t.minPlayers > playerCount));
      // Remove roles that are no longer available at this player count
      const filtered = prev.filter(r => TOGGLEABLE_ROLES.find(t => t.role === r)?.minPlayers! <= playerCount);
  ...
  useEffect(() => { localStorage.setItem('masoi-providers', JSON.stringify(providers)); }, [providers]);
  ...
  const updateProvider = (form: Partial<ProviderConfig>) => {
    setProviders(prev => prev.map(p => p.id === form.id ? { ...p, ...form } as ProviderConfig : p));
    setEditingId(null);
  };
  ...
  const testProvider = async (p: ProviderConfig) => {
    ...
    try {
      const res = await fetch('/api/providers/test', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(p) });
      const data = await res.json();
      setTestResults(r => ({ ...r, [p.id]: data.success }));
    } catch { setTestResults(r => ({ ...r, [p.id]: false })); }
  };
    ...
    <div className="w-full h-full overflow-y-auto bg-gradient-to-b from-[#0a0e1a] via-[#111827] to-[#0a0e1a]">
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        ...
        <div className="text-center py-8">
          <h1 className="text-5xl font-bold bg-gradient-to-r from-amber-400 via-red-500 to-purple-500 bg-clip-text text-transparent">🐺 Ma Sói 3D</h1>
          <p className="text-gray-400 mt-2">AI Đấu Trí Ma Sói — Xem trí tuệ nhân tạo chơi trò lừa dối</p>
        ...
        <section className="bg-gray-800/50 rounded-xl p-6 border border-gray-700">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={gameName} onChange={e => setGameName(e.target.value)} className="w-full bg-gray-900 border border-gray-600 rounded-lg px-3 py-2 text-white" />
            </div>
            <div>
              <label className="block text-sm text-gray-400 mb-1">Số Người Chơi: {playerCount}</label>
              <input type="range" min={6} max={16} value={playerCount} onChange={e => setPlayerCount(+e.target.value)} className="w-full accent-amber-400" />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="range" min={1000} max={10000} step={500} value={phaseDelay} onChange={e => setPhaseDelay(+e.target.value)} className="w-full accent-amber-400" />
            </div>
          </div>
          <div className="mt-4 flex items-center gap-3">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={autoPlay} onChange={e => setAutoPlay(e.target.checked)} className="accent-amber-400 w-4 h-4" />
              <span className="text-sm text-gray-300">Tự động chơi (bỏ tick để điều khiển từng bước)</span>
            </label>
          </div>
        </section>
        ...
        <section className="bg-gray-800/50 rounded-xl p-6 border border-gray-700">
          ...
          <p className="text-xs text-gray-500 mb-4">Bật/tắt vai trò đặc biệt. Sói Thường, Dân, và Tiên Tri luôn có mặt.</p>
          ...
          <div className="grid grid-cols-2 gap-2 mb-4">
            {TOGGLEABLE_ROLES.map(tr => {
              ...
              return (
                ...
                >
                  {/* Toggle indicator */}
                  <div className={`w-9 h-5 rounded-full flex items-center shrink-0 transition-colors duration-200 ${
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-sm font-medium ${enabled ? 'text-white' : 'text-gray-500'}`}>{tr.label}</span>
                      {!available && <span className="text-[10px] text-gray-600">({tr.minPlayers}+ người)</span>}
                    </div>
          ...
          <div className="border-t border-gray-700 pt-3">
            ...
            <div className="flex flex-wrap gap-2">
              {Object.entries(roleCounts).map(([role, count]) => (
                <div key={role} className={`flex items-center gap-1.5 bg-gray-900 rounded-lg px-3 py-1.5 border border-gray-700 ${ROLE_COLORS[role as Role] || 'text-gray-400'}`}>
                  ...
                  <span className="bg-gray-700 rounded-full px-1.5 py-0.5 text-[10px]">×{count}</span>
                </div>
              ))}
            </div>
          </div>

          <p className="text-xs text-gray-600 mt-3">🔄 Game loop: Đêm → Rạng Sáng → Thảo Luận → Hoàng Hôn (vote) → Phán Xét (biện hộ + giết/tha)</p>
        </section>
        ...
        <section className="bg-gray-800/50 rounded-xl p-6 border border-gray-700">
          ...
          {providers.length === 0 && !showAddForm && <p className="text-gray-500 text-sm">Chưa có nhà cung cấp nào. Thêm một cái để bắt đầu.</p>}
          {providers.map(p => (
              ...
              {editingId === p.id ? (
                ...
                <div className="flex items-center justify-between bg-gray-900 rounded-lg px-4 py-3 border border-gray-600">
                  <div className="min-w-0">
                    ...
                    <span className="text-gray-400 text-sm ml-2">({p.type} / {p.model})</span>
                    {p.baseUrl && <span className="text-gray-500 text-xs ml-2 truncate">{p.baseUrl}</span>}
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    {testResults[p.id] === true && <span className="text-green-400 text-sm">✓ Hoạt động</span>}
                    {testResults[p.id] === false && <span className="text-red-400 text-sm">✗ Lỗi</span>}
                    {testResults[p.id] === null && <span className="text-yellow-400 text-sm animate-pulse">Đang test...</span>}
                    <button onClick={() => testProvider(p)} className="bg-gray-700 hover:bg-gray-600 text-white px-3 py-1 rounded text-sm transition">Test</button>
                    <button onClick={() => { setEditingId(p.id); setShowAddForm(false); }} className="bg-gray-700 hover:bg-gray-600 text-white px-3 py-1 rounded text-sm transition">✎ Sửa</button>
                    <button onClick={() => setProviders(providers.filter(x => x.id !== p.id))} className="bg-red-900/50 hover:bg-red-800 text-red-300 px-3 py-1 rounded text-sm transition">✕</button>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="text-center pb-8">
          <button onClick={handleStart} disabled={!connected || !providers.length}
